...
int MaterialDraw(const S_BOARD *pos)
{
    ...
    if (!pos->pceNum[wR] && !pos->pceNum[bR] && !pos->pceNum[wQ] && !pos->pceNum[bQ])
    {
        ...
        else if ((pos->pceNum[wN] < 3 && !pos->pceNum[wB]) || (!pos->pceNum[wN] && pos->pceNum[wB] == 1))
        {
            if ((pos->pceNum[bN] < 3 && !pos->pceNum[bB]) || (!pos->pceNum[bN] && pos->pceNum[bB] == 1))
            {
                return TRUE;
            }
        }
...
int EvalPosition(const S_BOARD *pos)
{
    ...
    for (pceNum = 0; pceNum < pos->pceNum[pce]; ++pceNum)
    {
        ...
        ASSERT(SqOnBoard(sq));
        ASSERT(MIRROR64(SQ64(sq)) >= 0 && MIRROR64(SQ64(sq)) <= 63);
        score -= KnightsTable[MIRROR64(SQ64(sq))];
    }
    ...
    {
        score -= KingTableE[MIRROR64(SQ64(sq))];
    }
    else
    {
        score -= KingTableO[MIRROR64(SQ64(sq))];
    }
